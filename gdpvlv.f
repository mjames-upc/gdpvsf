...
C
     ...
     +			THEN
C                            print *,'ogrid/igrid: beyond range'
			    ogrid ( j, 1 ) = RMISSD
			    igrid ( j ) = RMISSD
		    ELSE IF ( ( idir .eq. 1 ) .and. 
     +				( grid(j) .gt. ylev ) )  THEN
C                            print *,'pgrid: beyond range'
C                            print *,'grid(j) is ', grid(j)
C                            print *,'ylev is ',ylev
		            pgrid (j) = RMISSD
		    ELSE
C
...
C
			...
			IF  ( presgrid(j) .gt. pmax ) THEN
                          print *, "beneath the lowest pressure level"
                          print *,"presgrid(j) is ",presgrid(j)
                          print *,"pmax is ", pmax
C			  pgrid (j) = RMISSD
			ELSE
			  pgrid (j) = grid (j)
...
C
C			IF  ( presgrid (j) .GT. pmax )  grid (j) = RMISSD
			IF  ( presgrid (j) .GT. pmax )  grid (j) = ystop
		      END IF			     
C
C*		      Is grid function missing?
C
		      IF  ( grid (j) .eq. RMISSD ) THEN
C                        print *, "grid function missing"
			pgrid (j) = RMISSD
		      ELSE
C
